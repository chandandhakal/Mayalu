...
export async function greet(language = 'en') {
...
export async function respond(text, language = 'en', sessionId) {
  if (!text || !text.trim()) {
...
export async function extractLead(sessionId) {
  const session = sessionId ? sessions[sessionId] : null;
  if (!session) return { lead: null };

  const lead = extractLeadFromHistory(session.messages, session.language);
  if (!lead.name && !lead.phone) return { lead: null };
